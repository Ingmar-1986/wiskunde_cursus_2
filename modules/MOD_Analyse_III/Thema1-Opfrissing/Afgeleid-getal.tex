\documentclass{ximera}

\input{../../../preamble.tex}
\input{../../../Opmaak.tex}

\title{Verandering en het afgeleid getal}

\begin{document}

% FVD_CHAPTER_DATA_START
% Automatisch gegenereerd uit index.tex.
% Niet handmatig aanpassen.
\fvdchapterdata{1}{Veranderingen in beeld}{1}
% FVD_CHAPTER_DATA_END




\begin{abstract}
In dit hoofdstuk frissen we een centraal idee uit de analyse op:
hoe beschrijf je hoe snel een functie verandert?

We vertrekken van de gemiddelde verandering over een interval en
herkennen daarin de helling van een secant. Door het interval steeds
kleiner te maken, komen we opnieuw bij de ogenblikkelijke verandering,
het afgeleid getal en de raaklijn.

Zo leggen we de basis om verderop nieuwe functies en nieuwe
afleidingstechnieken te onderzoeken.
\end{abstract}

\maketitle


% ============================================================
% VOORKENNIS EN LEERDOELEN
% ============================================================

\begin{voorkennis}
  \item Je kan functiewaarden berekenen.
  \item Je kent de richtingscoëfficiënt van een rechte.
  \item Je kan een differentiequotiënt berekenen.
  \item Je kent het begrip limiet.
  \item Je kent het afgeleid getal en de afgeleide functie.
  \item Je kan eenvoudige veeltermfuncties afleiden.
\end{voorkennis}

\begin{leerdoelen}
  \item Je kan een gemiddelde verandering berekenen en interpreteren.
  \item Je kan het differentiequotiënt meetkundig interpreteren
        als de richtingscoëfficiënt van een secant.
  \item Je kan gemiddelde en ogenblikkelijke verandering onderscheiden.
  \item Je kan het afgeleid getal interpreteren als een limiet
        van differentiequotiënten.
  \item Je kan het afgeleid getal meetkundig interpreteren als
        de richtingscoëfficiënt van de raaklijn.
  \item Je kan het teken en de eenheid van een afgeleid getal
        interpreteren.
  \item Je kan uit $f(a)$ en $f'(a)$ de vergelijking van de
        raaklijn aan de grafiek in $x=a$ bepalen.
\end{leerdoelen}

\FVDStartPositionFigure{-6}{6}{5}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% WAAROM LEER JE DIT?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Waarom leer je dit?}

In wetenschappen beschrijven functies vaak grootheden die veranderen.

Een temperatuur stijgt of daalt.
De hoeveelheid van een stof verandert tijdens een reactie.
Een populatie groeit.
Een elektrische spanning varieert in de tijd.
Een meetwaarde bereikt een maximum of minimum.

Een functie vertelt ons welke waarde een grootheid heeft.
Maar in veel situaties willen we nog iets anders weten:

\begin{center}
\textbf{Hoe verandert die grootheid?}
\end{center}

Daarvoor gebruiken we afgeleiden.

Je hebt het afgeleid getal en de afgeleide van veeltermfuncties
al in het vijfde jaar bestudeerd. In dit hoofdstuk bouwen we die
theorie daarom niet opnieuw volledig op.

We halen vooral de fundamentele verbanden opnieuw naar boven:

\[
\boxed{\text{gemiddelde verandering}}
\longleftrightarrow
\boxed{\text{secant}}
\]

en

\[
\boxed{\text{ogenblikkelijke verandering}}
\longleftrightarrow
\boxed{\text{raaklijn}}
\longleftrightarrow
\boxed{\text{afgeleid getal}}.
\]

Die verbanden zullen we in de volgende hoofdstukken telkens opnieuw
gebruiken wanneer we afgeleiden van nieuwe soorten functies onderzoeken.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ONTDEKKEN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Ontdekken}

\begin{exploration}{Hoe verandert een functie?}

Beschouw de functie

\[
f(x)=x^2.
\]

We onderzoeken de verandering van deze functie in de buurt van $x=1$.

Neem eerst het interval $[1,3]$.

\[
\frac{f(3)-f(1)}{3-1}
=
\frac{9-1}{2}
=
4.
\]

Neem daarna het kleinere interval $[1,2]$.

\[
\frac{f(2)-f(1)}{2-1}
=
\frac{4-1}{1}
=
3.
\]

Nog dichter bij $1$ krijgen we bijvoorbeeld

\[
\frac{f(1,5)-f(1)}{1,5-1}
=
\frac{2,25-1}{0,5}
=
2,5.
\]

En voor $x=1,1$:

\[
\frac{f(1,1)-f(1)}{1,1-1}
=
\frac{1,21-1}{0,1}
=
2,1.
\]

\begin{question}
Naar welke waarde lijken deze differentiequotiënten te naderen
wanneer de tweede $x$-waarde steeds dichter bij $1$ komt?

\[
\answer{2}
\]
\end{question}

\begin{question}
Welke uitspraak past het best bij die waarde?

\begin{multipleChoice}
  \choice{Het is de functiewaarde $f(1)$.}
  \choice{Het is de gemiddelde verandering over $[1,3]$.}
  \choice[correct]{Het is de ogenblikkelijke verandering van $f$
                   bij $x=1$.}
  \choice{Het is de nulwaarde van $f$.}
\end{multipleChoice}
\end{question}

\end{exploration}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% WAT MERK JE?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Wat merk je?}

Wanneer we twee verschillende punten van een grafiek nemen,
krijgen we een \textbf{secant}.

De helling van die secant beschrijft een
\textbf{gemiddelde verandering}.

Wanneer het tweede punt steeds dichter bij het eerste punt komt,
wordt het interval steeds kleiner.

De secant nadert dan naar de \textbf{raaklijn} in het vaste punt.

Tegelijk gaat de gemiddelde verandering over in een
\textbf{ogenblikkelijke verandering}.

Dit proces vatten we samen als

\[
\boxed{
\text{secant}
\longrightarrow
\text{raaklijn}
}
\]

en

\[
\boxed{
\text{gemiddelde verandering}
\longrightarrow
\text{ogenblikkelijke verandering}
}.
\]

De wiskundige techniek waarmee we deze overgang exact beschrijven,
is een \textbf{limiet}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% GEMIDDELDE VERANDERING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Gemiddelde verandering}

\xmsubsection{Verandering over een interval}

Beschouw een functie $f$ en twee waarden $a$ en $b$.

De verandering van de onafhankelijke veranderlijke is

\[
\Delta x=b-a.
\]

De verandering van de functiewaarde is

\[
\Delta f=f(b)-f(a).
\]

Om te weten hoeveel de functiewaarde gemiddeld verandert per
eenheid verandering van $x$, vergelijken we beide veranderingen.

\begin{definition}
De \textbf{gemiddelde verandering} van een functie $f$
over het interval $[a,b]$ is

\[
\important{
\frac{\Delta f}{\Delta x}
=
\frac{f(b)-f(a)}{b-a}
}.
\]

Deze verhouding noemen we het \textbf{differentiequotiënt}.
\end{definition}


\begin{example}

Beschouw

\[
f(x)=-x^2+8x.
\]

Bepaal de gemiddelde verandering over $[2,4]$.

\begin{oplossing}

We berekenen eerst

\[
f(2)=-2^2+8\cdot2=12
\]

en

\[
f(4)=-4^2+8\cdot4=16.
\]

Daarom is

\[
\frac{f(4)-f(2)}{4-2}
=
\frac{16-12}{2}
=
2.
\]

De functiewaarde neemt tussen $x=2$ en $x=4$
\textbf{gemiddeld met $2$ eenheden toe per eenheid toename van $x$}.

\end{oplossing}

\end{example}


\begin{exercise}

Voor een functie $f$ geldt

\[
f(3)=7
\qquad\text{en}\qquad
f(8)=2.
\]

Bereken de gemiddelde verandering over $[3,8]$.

\[
\frac{f(8)-f(3)}{8-3}
=
\answer{-1}.
\]

Welke interpretatie is correct?

\begin{multipleChoice}
  \choice{De functie daalt overal met precies $1$.}
  \choice[correct]{Tussen $x=3$ en $x=8$ neemt de functiewaarde
  gemiddeld met $1$ af per eenheid toename van $x$.}
  \choice{De functiewaarde is gelijk aan $-1$.}
  \choice{Elke raaklijn aan de grafiek heeft richtingscoëfficiënt $-1$.}
\end{multipleChoice}

\end{exercise}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SECANT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\xmsubsection{Meetkundige betekenis}

Beschouw de punten

\[
A(a,f(a))
\qquad\text{en}\qquad
B(b,f(b))
\]

op de grafiek van $f$.

De rechte door $A$ en $B$ is een \textbf{secant}.

De richtingscoëfficiënt van deze secant is

\[
\operatorname{rico}(AB)
=
\frac{f(b)-f(a)}{b-a}.
\]

Dat is precies het differentiequotiënt.

\begin{definition}
Het differentiequotiënt is de
\textbf{richtingscoëfficiënt van de secant}
door de twee overeenkomstige punten van de grafiek.

\[
\important{
\text{gemiddelde verandering}
\longleftrightarrow
\text{richtingscoëfficiënt van een secant}
}
\]
\end{definition}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% EENHEDEN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\xmsubsection{Eenheden}

Bij toepassingen heeft een differentiequotiënt meestal ook een eenheid.

Stel bijvoorbeeld dat $T(t)$ de temperatuur in graden Celsius
voorstelt op tijdstip $t$, uitgedrukt in seconden.

Dan heeft

\[
\frac{\Delta T}{\Delta t}
\]

als eenheid

\[
\frac{^\circ\mathrm{C}}{\mathrm{s}}.
\]

Als

\[
\frac{\Delta T}{\Delta t}=-0,8,
\]

dan betekent dit dat de temperatuur over het onderzochte interval
\textbf{gemiddeld met $0,8~^\circ\mathrm{C}$ per seconde afneemt}.

\begin{remark}
Bij een toepassing is een getal alleen meestal geen volledig antwoord.

Vermeld indien mogelijk:

\begin{itemize}
  \item welke grootheid verandert;
  \item of de grootheid toeneemt of afneemt;
  \item hoeveel de grootheid verandert;
  \item per welke eenheid;
  \item over welk interval de uitspraak geldt.
\end{itemize}
\end{remark}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% OGENBLIKKELIJKE VERANDERING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Van gemiddelde naar ogenblikkelijke verandering}

\xmsubsection{Het interval steeds kleiner maken}

We kiezen een vast punt

\[
A(a,f(a))
\]

op de grafiek van $f$.

Daarnaast nemen we een tweede punt

\[
B(x,f(x)).
\]

Voor $x\neq a$ heeft de secant $AB$ richtingscoëfficiënt

\[
\frac{f(x)-f(a)}{x-a}.
\]

Nu laten we $x$ steeds dichter bij $a$ komen:

\[
x\to a.
\]

Het punt $B$ nadert dan tot $A$.

De secant door $A$ en $B$ nadert naar de raaklijn aan de grafiek
in $A$.

We onderzoeken daarom

\[
\lim_{x\to a}
\frac{f(x)-f(a)}{x-a}.
\]

Deze limiet beschrijft niet langer een gemiddelde verandering
over een interval, maar de \textbf{ogenblikkelijke verandering}
bij $x=a$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% AFGELEID GETAL
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Het afgeleid getal}

\begin{definition}
Als de limiet

\[
\lim_{x\to a}
\frac{f(x)-f(a)}{x-a}
\]

bestaat en een reëel getal is, dan is $f$
\textbf{afleidbaar in $a$}.

Het \textbf{afgeleid getal van $f$ in $a$} is

\[
\important{
f'(a)
=
\lim_{x\to a}
\frac{f(x)-f(a)}{x-a}
}.
\]
\end{definition}

We kunnen dezelfde definitie schrijven met

\[
x=a+h.
\]

Wanneer $x\to a$, geldt $h\to0$.

Daarom is ook

\[
\important{
f'(a)
=
\lim_{h\to0}
\frac{f(a+h)-f(a)}{h}
}.
\]


\xmsubsection{Een voorbeeld met de definitie}

\begin{example}

Gegeven is

\[
f(x)=x^2+2x.
\]

Bepaal $f'(1)$ met de definitie.

\begin{oplossing}

We gebruiken

\[
f'(1)
=
\lim_{x\to1}
\frac{f(x)-f(1)}{x-1}.
\]

Omdat

\[
f(1)=3,
\]

volgt

\begin{align*}
f'(1)
&=
\lim_{x\to1}
\frac{x^2+2x-3}{x-1}
\\
&=
\lim_{x\to1}
\frac{(x-1)(x+3)}{x-1}
\\
&=
\lim_{x\to1}(x+3)
\\
&=4.
\end{align*}

Dus

\[
\boxed{f'(1)=4}.
\]

\end{oplossing}

\end{example}

De berekening is slechts één deel van het antwoord.

De waarde

\[
f'(1)=4
\]

betekent ook dat de ogenblikkelijke verandering van $f$
bij $x=1$ gelijk is aan $4$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% RAAKLIJN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{De afgeleide en de raaklijn}

\xmsubsection{Meetkundige betekenis}

De overgang van secant naar raaklijn geeft onmiddellijk de
meetkundige betekenis van het afgeleid getal.

\begin{definition}
Het afgeleid getal $f'(a)$ is de
\textbf{richtingscoëfficiënt van de raaklijn}
aan de grafiek van $f$ in het punt

\[
P(a,f(a)).
\]

Dus

\[
\important{
f'(a)=\operatorname{rico}(t)
}
\]

waarbij $t$ de raaklijn is in $P$.
\end{definition}


\xmsubsection{Wat vertelt het teken?}

Als

\[
f'(a)>0,
\]

heeft de raaklijn een positieve richtingscoëfficiënt.

Als

\[
f'(a)<0,
\]

heeft de raaklijn een negatieve richtingscoëfficiënt.

Als

\[
f'(a)=0,
\]

is de raaklijn horizontaal.

\begin{remark}
Uit

\[
f'(a)=0
\]

mag je niet onmiddellijk besluiten dat $f$ in $a$
een maximum of minimum bereikt.

Een horizontale raaklijn is op zichzelf onvoldoende om een
extremum te besluiten.
\end{remark}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% INTERPRETATIE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Een afgeleid getal interpreteren}

Bij een toepassing moeten we opnieuw rekening houden met de betekenis
en de eenheden van de grootheden.

Stel dat $Q(t)$ een hoeveelheid in milligram voorstelt en dat
$t$ in uren wordt uitgedrukt.

Als

\[
Q'(4)=-2,5,
\]

dan heeft de afgeleide als eenheid

\[
\frac{\mathrm{mg}}{\mathrm{uur}}.
\]

Een volledige interpretatie is:

\begin{center}
Op het tijdstip $t=4$ uur neemt de hoeveelheid $Q$
ogenblikkelijk af met $2,5$ milligram per uur.
\end{center}

Bij een interpretatie stellen we dus drie vragen:

\begin{enumerate}
  \item \textbf{Waar?} Bij welke invoerwaarde?
  \item \textbf{In welke richting?} Toename, afname of horizontaal?
  \item \textbf{Hoe sterk?} Met welke waarde en welke eenheid?
\end{enumerate}


\begin{exercise}

De functie $C(t)$ beschrijft een concentratie in $\mathrm{mg/L}$,
waarbij $t$ wordt uitgedrukt in minuten.

Er geldt

\[
C'(12)=-0,30.
\]

Welke interpretatie is correct?

\begin{multipleChoice}

\choice{
Na $12$ minuten bedraagt de concentratie
$-0,30~\mathrm{mg/L}$.
}

\choice{
Gedurende de eerste $12$ minuten daalt de concentratie
gemiddeld met $0,30~\mathrm{mg/L}$.
}

\choice[correct]{
Op het tijdstip $t=12$ minuten neemt de concentratie
ogenblikkelijk af met $0,30~\mathrm{mg/L}$ per minuut.
}

\choice{
Na $12$ minuten is de concentratie nul.
}

\end{multipleChoice}

\end{exercise}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% RAAKLIJNVERGELIJKING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{De vergelijking van de raaklijn}

De raaklijn aan de grafiek in

\[
P(a,f(a))
\]

heeft richtingscoëfficiënt

\[
f'(a).
\]

Een rechte met richtingscoëfficiënt $m$ door een punt
$(x_1,y_1)$ heeft vergelijking

\[
y-y_1=m(x-x_1).
\]

Daaruit volgt:

\begin{definition}
De raaklijn aan de grafiek van $f$ in het punt
$P(a,f(a))$ heeft vergelijking

\[
\important{
y-f(a)=f'(a)(x-a)
}.
\]
\end{definition}


\begin{example}

Gegeven is

\[
f(x)=x^2+2x.
\]

We weten dat

\[
f(1)=3
\qquad\text{en}\qquad
f'(1)=4.
\]

De raaklijn bij $x=1$ heeft dus vergelijking

\begin{align*}
y-3 &= 4(x-1)
\\
y-3 &= 4x-4
\\
y &= 4x-1.
\end{align*}

Dus

\[
\boxed{y=4x-1}.
\]

\end{example}


\begin{exercise}

Voor een functie $f$ geldt

\[
f(2)=5
\qquad\text{en}\qquad
f'(2)=-3.
\]

Vul aan.

De raaklijn gaat door

\[
P=(\answer{2},\answer{5})
\]

en heeft richtingscoëfficiënt

\[
\answer{-3}.
\]

De vergelijking is

\[
y-5=-3(x-2),
\]

dus

\[
y=\answer{-3}x+\answer{11}.
\]

\end{exercise}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% VERBANDEN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{De verbanden opnieuw samenbrengen}

De belangrijkste begrippen uit dit hoofdstuk kunnen we in twee
ketens samenvatten.

Voor twee verschillende punten:

\[
\important{
\text{differentiequotiënt}
\longleftrightarrow
\text{gemiddelde verandering}
\longleftrightarrow
\text{secant}
}
\]

Voor één punt:

\[
\important{
\text{afgeleid getal}
\longleftrightarrow
\text{ogenblikkelijke verandering}
\longleftrightarrow
\text{raaklijn}
}
\]

De overgang tussen beide gebeurt door een limiet:

\[
\boxed{
f'(a)
=
\lim_{x\to a}
\frac{f(x)-f(a)}{x-a}
}.
\]


\xmsubsection{Drie talen van de afgeleide}

Eenzelfde uitspraak over een afgeleide kunnen we op verschillende
manieren formuleren.

\begin{center}
\begin{tabular}{p{0.25\textwidth}|p{0.62\textwidth}}
\textbf{Algebraïsch}
&
$f'(a)$ is de limiet van de differentiequotiënten.
\\[2mm]

\textbf{Meetkundig}
&
$f'(a)$ is de richtingscoëfficiënt van de raaklijn bij $x=a$.
\\[2mm]

\textbf{In context}
&
$f'(a)$ beschrijft de ogenblikkelijke verandering van de
afhankelijke grootheid bij $x=a$.
\end{tabular}
\end{center}

Bij analyse moet je tussen deze drie voorstellingen kunnen schakelen.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% DENKFOUTEN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Veelgemaakte denkfouten}

\begin{remark}
\textbf{Gemiddelde en ogenblikkelijke verandering verwarren}

\[
\frac{f(b)-f(a)}{b-a}
\]

beschrijft een verandering over een interval.

\[
f'(a)
\]

beschrijft een ogenblikkelijke verandering bij één invoerwaarde.
\end{remark}


\begin{remark}
\textbf{De afgeleide verwarren met de functiewaarde}

\[
f(a)
\]

vertelt waar het punt op de grafiek ligt.

\[
f'(a)
\]

vertelt hoe de grafiek daar ogenblikkelijk verandert.
\end{remark}


\begin{remark}
\textbf{Uit $f'(a)=0$ onmiddellijk een extremum besluiten}

Een horizontale raaklijn is niet noodzakelijk een maximum of minimum.
Het verloop rond het punt moet eveneens onderzocht worden.
\end{remark}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SLOTOPDRACHT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Kun je alles verbinden?}

\begin{exercise}

Van een functie $f$ weten we dat

\[
f(2)=7
\qquad\text{en}\qquad
f'(2)=-3.
\]

\begin{question}

Door welk punt gaat de raaklijn?

\[
P=(\answer{2},\answer{7}).
\]

\end{question}

\begin{question}

Wat is de richtingscoëfficiënt van de raaklijn?

\[
\operatorname{rico}(t)=\answer{-3}.
\]

\end{question}

\begin{question}

Hoe verloopt de raaklijn van links naar rechts?

\begin{multipleChoice}
  \choice{Ze stijgt.}
  \choice[correct]{Ze daalt.}
  \choice{Ze is horizontaal.}
\end{multipleChoice}

\end{question}

\begin{question}

Bepaal de vergelijking van de raaklijn.

\[
y-7=-3(x-2),
\]

dus

\[
y=\answer{-3}x+\answer{13}.
\]

\end{question}

\begin{question}

Een leerling beweert:

\begin{center}
``Omdat $f'(2)=-3$, geldt noodzakelijk $f(3)=4$.''
\end{center}

Is dat correct?

\begin{multipleChoice}

\choice{
Ja, want $7-3=4$.
}

\choice[correct]{
Nee. $f'(2)$ beschrijft de ogenblikkelijke verandering bij $x=2$.
Daaruit volgt niet dat de functie over het volledige interval
$[2,3]$ met precies $3$ afneemt.
}

\end{multipleChoice}

\end{question}

\end{exercise}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SAMENVATTING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Samengevat}

Over een interval $[a,b]$ gebruiken we het differentiequotiënt

\[
\boxed{
\frac{f(b)-f(a)}{b-a}
}.
\]

Dit beschrijft de gemiddelde verandering en is de
richtingscoëfficiënt van een secant.

Bij één punt gebruiken we het afgeleid getal

\[
\boxed{
f'(a)
=
\lim_{x\to a}
\frac{f(x)-f(a)}{x-a}
}.
\]

Dit beschrijft de ogenblikkelijke verandering en is de
richtingscoëfficiënt van de raaklijn bij $x=a$.

De vergelijking van die raaklijn is

\[
\boxed{
y-f(a)=f'(a)(x-a)
}.
\]

Bij toepassingen interpreteren we een afgeleid getal altijd
in functie van de grootheden, het teken en de eenheden.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% MINIMUMTRAJECT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\FVDsection{Wat moet je zeker beheersen?}

Na deze korte opfrissing moet je zelfstandig:

\begin{itemize}
  \item een differentiequotiënt berekenen en interpreteren;
  \item het verband leggen tussen differentiequotiënt en secant;
  \item gemiddelde en ogenblikkelijke verandering onderscheiden;
  \item het afgeleid getal herkennen als een limiet van
        differentiequotiënten;
  \item het verband leggen tussen afgeleid getal en raaklijn;
  \item een afgeleid getal in een wetenschappelijke context
        interpreteren, met teken en eenheid;
  \item uit $f(a)$ en $f'(a)$ de vergelijking van een raaklijn bepalen;
  \item uitleggen waarom $f'(a)=0$ op zichzelf nog geen extremum bewijst.
\end{itemize}

Dit is de noodzakelijke basis voor de volgende hoofdstukken,
waarin we deze ideeën toepassen op nieuwe functieklassen en
nieuwe afleidingsregels.

\end{document}