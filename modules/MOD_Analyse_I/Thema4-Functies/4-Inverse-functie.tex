...
\begin{opmerking}
Dit hoofdstuk realiseert de kern van het leerplandoel over het
\textbf{grafische verband tussen inverteerbare functies en hun inverse}.
De minimale afbakening vermeldt daarbij expliciet \textbf{bijectie}.

Cyclometrische functies behoren eveneens tot die minimale afbakening, maar
worden pas volledig behandeld nadat sinus, cosinus en tangens als functies
zijn opgebouwd. Het principe van domeinbeperking dat daarvoor nodig is,
hebben we in hoofdstuk 3 al voorbereid.
\end{opmerking}
...
\begin{itemize}
    \item Een inverse functie keert de werking van een bijectieve functie om.
    \item
    \[
    f(x)=y\Longleftrightarrow f^{-1}(y)=x.
    \]
    \item \(f^{-1}\) betekent niet \(1/f\).
    \item Domein en bereik wisselen van rol:
    \[
    \operatorname{dom}f^{-1}=\operatorname{ber}f,
    \qquad
    \operatorname{ber}f^{-1}=\operatorname{dom}f.
    \]
    \item Een punt \((a,b)\) op \(f\) correspondeert met \((b,a)\) op \(f^{-1}\).
    \item Daarom zijn beide grafieken elkaars spiegelbeeld in \(y=x\).
    \item Bij eenvoudige functies kan je het inverse voorschrift vinden door
          invoer en uitvoer te verwisselen en opnieuw naar de uitvoer op te lossen.
    \item Een inverse kan gecontroleerd worden via
    \[
    f^{-1}(f(x))=x
    \quad\text{en}\quad
    f(f^{-1}(x))=x.
    \]
    \item Een niet-bijectieve functie moet eventueel eerst een passend beperkt
          domein en codomein krijgen.
\end{itemize}
...
\begin{itemize}
    \item uitleggen waarom bijectiviteit nodig is voor een inverse functie;
    \item het grafische verband tussen \(f\) en \(f^{-1}\) analyseren;
    \item een inverse grafiek construeren door spiegeling in \(y=x\);
    \item domein en bereik van \(f\) en \(f^{-1}\) met elkaar verbinden;
    \item eenvoudige inverse voorschriften bepalen;
    \item je antwoord controleren door heen en terug te gaan;
    \item een inverse functie betekenisvol interpreteren in een context.
\end{itemize}

De noodzakelijke oefeningen zijn gemarkeerd met
\textbf{\(\blacklozenge\) Essentieel}.
\end{opmerking}
