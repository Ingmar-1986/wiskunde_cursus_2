...
Je kunt:
\begin{itemize}
    \item een functievoorschrift interpreteren;
    \item domein, beelden en originelen bepalen;
    \item vergelijkingen oplossen;
    \item eenvoudige veeltermen ontbinden;
    \item een grafiek lezen en punten met hun coördinaten verbinden.
\end{itemize}
\end{voorkennis}
...
\begin{itemize}
    \item nulwaarden en nulpunten onderscheiden;
    \item nulwaarden grafisch aflezen en algebraïsch bepalen;
    \item uit een grafiek aflezen waar \(f(x)>0\), \(f(x)=0\) en \(f(x)<0\);
    \item een tekenverloop onderzoeken en in een tekentabel samenvatten;
    \item rekening houden met waarden die niet tot het domein behoren;
    \item grafiek, voorschrift en tekentabel met elkaar verbinden;
    \item verklaren waarom een nulwaarde niet noodzakelijk een tekenwisseling veroorzaakt;
    \item het teken van een functie in een betekenisvolle context interpreteren.
\end{itemize}
\end{leerdoelen}
